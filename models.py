...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, img_size, var=None):
        ...
        if self.training:
            ...
            # p_cls = F.softmax(p[:, 5:6], 1) * p_conf
            ...
            # onnx算子不支持pytorch的部分算子
            # 修改版本
            grid_xy = self.grid_xy.repeat((1, self.na, 1, 1, 1)).view((1, -1, 2))
            anchor_wh = self.anchor_wh.repeat((1, 1, self.nx, self.ny, 1)).view((1, -1, 2))
            p = p.view(-1, 5 + self.nc)
            xy = torch.sigmoid(p[..., 0:2]) + grid_xy[0]  # x, y
            wh = torch.exp(p[..., 2:4]) * anchor_wh[0]  # width, height
            temp = torch.cat((xy, wh), 1)
            temp1 = temp[..., :4] * self.stride
            p_conf = torch.sigmoid(p[..., 4:5])
            p_cls = torch.ones_like(p[..., 5], dtype=torch.float).view(-1, 1)
            # p_cls = p[..., 5:6]
            # p_cls = p_cls.view(-1, 1)
            # p_cls -= p_cls
            # p_cls += 1
            return torch.cat((temp1, p_conf, p_cls), 1).t()
...
class Darknet(nn.Module):
    ...
    def forward(self, x, var=None):
        ...
        if self.training:
            ...
            # return output[4:5 + nc].t(), output[:4].t()
